    ...
    <body>
        ...
        <div id="page-container" class="sidebar-o side-scroll main-content-boxed side-trans-enabled page-header-fixed">
            ...
            <main id="main-container">
                <!-- Page Content -->
                <div class="content">
                <form action="<?php echo base_url().'admin/paten/update_pt'?>" method="post" enctype="multipart/form-data">
                    <div class="row gutters-tiny invisible" data-toggle="appear">
                        <div class="col-md-8">
                            <div class="block">
                                <div class="block-header block-header-default">
                                    <h3 class="block-title">Edit Paten</h3>
                                </div>
                                <div class="block-content">
                                    
                                    <div class="form-group">
                                            <input type="text" name="xnama" value="<?php echo $b['pt_nama'];?>" class="form-control" placeholder="Nama" required>
                                    </div>
                                    <div class="form-group">
                                            <input type="text" name="xikm" value="<?php echo $b['pt_perusahaan'];?>" class="form-control" placeholder="Nama" required>
                                    </div>
                                    <div class="form-group">
                                            <input type="text" name="xemail" value="<?php echo $b['pt_email'];?>" class="form-control" placeholder="Email" readonly>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="input-group-addon position-absolute">
                                            ...
                                            <input type="tel" name="xnohp" value="<?php echo $b['pt_nohp'];?>" class="form-control" placeholder="No. Handphone" onkeypress="return event.charCode >= 48 && event.charCode <= 57" required>
                                    </div>
                                    </div>

                                    <div class="form-group">
                                        <label class="control-label">Alamat:</label>
                                            <textarea name="xalamat" id="xalamat" class="form-control" rows="4" maxlength="300" required oninvalid="this.setCustomValidity('Wajib Diisi')" oninput="setCustomValidity('')"><?php echo $b['pt_alamat']; ?></textarea>
                                            <div><span id="xsalamat"></span></div>
                                    </div>

                                  <div class="form-group">
                                         <?php if($this->session->userdata('akses')=='1'):?>
                                        <label class="control-label">Status:</label><br>
                                         <select name="pt_statusid" id="pt_statusid" class="form-control opt1hid" readonly>
                                     <option value="" disabled selected hidden>------------------------------- Status -------------------------------</option>
                                      <?php $status = $this->db->query("SELECT * FROM status ORDER BY status_nama ASC")->result(); ?>
                                        <?php foreach ($status as $pr) { ?>
                                        <?php if($b['pt_statusid'] == 0) { ?>
                                            <option value="<?php echo $pr->status_id; ?>"><?php echo $pr->status_nama; ?></option>
                                        <?php } else { ?>
                                            <option value="<?php echo $pr->status_id; ?>" <?php echo ($pr->status_id == $b['pt_statusid']) ? "selected" : ""; ?>>
                                      ...
                                      <?php }; ?>
                                        ...
                                        <label class="control-label" hidden>Status:</label><br>
                                         <select name="pt_statusid" id="pt_statusid" class="form-control opt1hid" hidden>
                                     <option value="" disabled selected hidden>------------------------------- Status -------------------------------</option>
                                      <?php $status = $this->db->query("SELECT * FROM status ORDER BY status_nama ASC")->result(); ?>
                                        <?php foreach ($status as $pr) { ?>
                                        <?php if($b['pt_statusid'] == 0) { ?>
                                    ...
                                    <div class="form-group">
                                     ...
                                     <option value="" disabled selected hidden>Pilih Provinsi</option>
                                      <?php $provinsi = $this->db->query("SELECT * FROM tb_provinsi ORDER BY nama ASC")->result(); ?>
                                        ...
                                        <?php if($b['pt_idprovinsi'] == 0) { ?>
                                            <option value="<?php echo $pr->id; ?>"><?php echo $pr->nama; ?></option>
                                        <?php } else { ?>
                                            <option value="<?php echo $pr->id; ?>" <?php echo ($pr->id == $b['pt_idprovinsi']) ? "selected" : ""; ?>>
                                                <?php echo $pr->nama; ?>
                                            </option>
                                        <?php }; ?>
                                      <?php }; ?>
                                ...
                                <div class="form-group">
                                     ...
                                     <option value="<?php echo $kb->id; ?>" <?php echo ($kb->id == $b['pt_idkabkota']) ? "selected" : ""; ?>>
                                         <?php echo $kb->nama; ?>
                                     </option>
                                      <?php }; ?>
                                     </select>
                                 ...
                                 <div class="form-group">
                                    <label class="control-label">Kode Pos:</label>
                                        <input type="text" name="pt_kodepos" id="pt_kodepos" value="<?php echo $b['pt_kodepos']; ?>" class="form-control">
                        ...
                        <div class="col-md-4">
                            <div class="block">
                                <div class="block-header block-header-default">
                                    <h3 class="block-title">Gambar Paten</h3>
                                </div>
                                <div class="block-content">
                                    <div class="form-group">
                                        <input type="file" name="filefoto" class="dropify2" data-height="230" data-default-file="<?php echo base_url().'assets/images/'.$b['pt_photo'];?>">
                                    </div>
                                    
                                    
                                </div>
                                <div class="block-content block-content-full block-content-md bg-body-light">
                                    <input type="hidden" name="kode" value="<?php echo $b['pt_id'];?>" required>
        ...
        $('#email').change(function() {
            ...
            if(email != '') {
                $.ajax({
                    url: "<?php echo base_url('admin/paten/cek_email'); ?>",
                    method : "post",
                    data   : { email:email },
